...
There are 3 problematic rows;

two with NAs for final fv/fm (nov pio 1, leaf 7 + jan mat 4, leaf 14 ), one with a wet_mass typo (envelope_id DKU2239). comments added for missing FvFm, change wet_mass typo with code?

### Creating unique plant ID
...
#OPTION 2
# each file separately, then bind to merge them

leaf_nov <- read_csv("leaf_scan_data/DURIN_Calluna_leaf_area_2025_November.csv",
                     show_col_types = FALSE) |>
  clean_names() |>
  mutate(source_file = "DURIN_Calluna_leaf_area_2025_November.csv")

leaf_jan <- read_csv("leaf_scan_data/DURIN_Calluna_leaf_area_2026_January.csv",
                     show_col_types = FALSE) |>
  clean_names() |>
  mutate(source_file = "DURIN_Calluna_leaf_area_2026_January.csv")

leaf_problem1 <- read_csv("leaf_scan_data/DURIN_Calluna_leaf_area_problem_images.csv",
                          show_col_types = FALSE) |>
  clean_names() |>
  mutate(source_file = "DURIN_Calluna_leaf_area_problem_images.csv")

leaf_problem2 <- read_csv("leaf_scan_data/DURIN_Calluna_leaf_area_problem_images2.csv",
                          show_col_types = FALSE) |>
  clean_names() |>
  mutate(source_file = "DURIN_Calluna_leaf_area_problem_images2.csv")

# combine all files
leaf_area_raw <- bind_rows(
  leaf_nov,
  leaf_jan,
  leaf_problem1,
  leaf_problem2
...
# rename leaf area column
leaf_area_clean <- leaf_area_raw |>
  rename(leaf_area_sum = leaf_area) |>
  select(source_file, x1, id, leaf_area_sum)
...
leaf_area_dir <- "leaf_scan_data" # location on my within the r project on my laptop

leaf_area_raw <- list.files(leaf_area_dir, pattern = "\\.csv$", full.names = TRUE)

names(leaf_area_raw) <- basename(leaf_area_raw)

leaf_area_raw <- purrr::map_df(  leaf_area_raw,
  ~ readr::read_csv(.x, show_col_types = FALSE),
  .id = "source_file"
)


# removing duplicate barcode_ids, that is now imported twice, once from  so "if "source_file" is DURIN_Calluna_leaf_area_2025_November.csv and "id" is DHK7373, or if  DURIN_Calluna_leaf_area_2026_January.csv and ASF3351
# removing by slicing the specific rows

leaf_area_clean <- leaf_area_raw |>
   clean_names() |>
   rename(leaf_area_sum = leaf_area) #|>
#   slice(-13) |> # remove nov duplicate 
#   slice (-268)  # remove jan duplicate 

view(leaf_area_clean)

```

### Calculating average leaf area per leaf

```{r}
#| label: calculating leaf area
#| message: false

# calculating leaf area per leaf by dividing summarized leaf area by the correct number of leaves on scan
leaf_area_clean <- leaf_area_clean |>
  mutate(
    leaf_area_mean = if_else(  # creating a new colum for mean leaf area per leaf
      !is.na(n_correct) & n_correct > 0,
      leaf_area_sum / n_correct,
      NA_real_
    )
  )
...
# mergning dataframes so that leaf scan data left joins by envelop ID

#data <- data |> 
# left join, keep values from x, and only matching rows in y (leaf_area_db)
# dplyr::left_join(leaf_area_db, by = "envelope_id")
...
data <- data |> 
  mutate(
    ...
    # adding and overwriting remarks for specific envelope_ids so that it matches writing on envelopes
    remark = case_when(
      envelope_id == "DMQ0002" ~ "one leaf has a bud",
      envelope_id == "AOW0486" ~ "only 3 leaves at scanning",
      envelope_id == "ASO5853" ~ "only 3 leaves at wet mass",
      TRUE ~ remark
    ),
  )


    # also, adding remark for missing FvFm remark from notebook
data <- data |> 
  mutate(
    remark = case_when(
      month == "November" &
...
#| message: false
 ...
 # if envelope_id is one of the following and bulk_nr is 3, then change it to 4
...
data <- data |> 
  mutate(
    bulk_nr_leaves = if_else(
      envelope_id %in% c("ATE6572", "ASO5853", "ARH6082", "AOL3307") &
      bulk_nr_leaves == 3,
      4,
      bulk_nr_leaves
    )
...
# making new columns 
data$bulk_nr_wet_mass <- data$bulk_nr_leaves # copying bulk_nr_leaves column, ready to be updated according to remarks regarding missing leaves at wet mass
...
data$wet_mass_leaf <- with(data, ifelse(
...
#### Leaf area (need to import first)
...
# making new columns 
data$bulk_nr_leaf_area <- data$bulk_nr_leaves # copying bulk_nr_leaves column, ready to be updated according to remarks regarding missing leaves at scanning 

data$remark_bulk_nr_leaf_area <- NA # making new remark column, ready for manual entries

# overwriting bulk_nr_leaf_area and adding update remark for rows with remarks regarding missing leaves at wet mass, then for leaves missing at scanning 
...
# calculating mean leaf area per leaf, unless there are NAs or 0s
data$leaf_area_mean <- with(data, ifelse(
  !is.na(bulk_nr_leaf_area) & bulk_nr_leaf_area > 0,
...
# making new columns 
data$bulk_nr_leaf_thickness <- data$bulk_nr_leaves # copying bulk_nr_leaves column, ready to be updated according to remarks regarding missing leaves at thickness 

data$remark_bulk_nr_leaf_thickness <- NA # making new remark column, ready for manual entries

# overwriting bulk_nr_leaf_thickness and adding update remark for rows with remarks regarding missing leaves at wet mass, then for leaves missing at scanning, then for missing leaves at  
...
# calculate mean thickness here or not?
...
# making new columns 
data$bulk_nr_dry_mass <- data$bulk_nr_leaves # copying bulk_nr_leaves column, ready to be updated according to remarks regarding missing leaves at dry mass
...
data$bulk_nr_dry_mass[data$remark == "only 2 leaves at thickness"] <- 2
...
# not sure if it actually necessary to make a column explicitly for this 
data$dry_mass_leaf <- with(data, ifelse(
...
ggplot(data |> filter(proj_2025 == "TT Hot"),
       aes(x = temp_exp, y = fv_fm_final, colour = factor(plant_nr))) +
  geom_point(size = 2, alpha = 0.8) +
  facet_grid(age_class ~ month) +
  theme_bw() +
  labs(
    x = "Temperature exposure (B0C)",
    y = "Fv/Fm final",
...
ggplot(data |> filter(proj_2025 == "TT Cold"),
       aes(x = temp_exp, y = fv_fm_final, colour = factor(plant_nr) )) +
  geom_point(size = 2, alpha = 0.8) +
  facet_grid(age_class ~ month) +
  theme_bw() +
...
ggplot(data |> filter(proj_2025 == "Traits"),
       aes(x = temp_exp, y = fv_fm_initial, colour = factor(plant_nr))) +
  geom_point(size = 2, alpha = 0.8, 
  ...
  labs(
    x = "Temperature exposure (B0C)",
    y = "Fv/Fm initial",
    title = "Fv/Fm initial by temperature exposure, age class and season"
  )
...
# need the data first, but will be the same principles 
...
group_vars_fin <- c("age_class", "month", "temp_exp", "plant_nr")

#TRIED ADDING plant_nr HERE BUT ALL THE FLAGGED PONITS DISAPPEARED. WHY? therefore removed again. seems to work on all othe variables
#might be just that there is no outliers - that is also a possibility, so put it back in during tues meeting 



# calculating the Inter Quartile Range (IQR) for a given trait, here fvfm final for hot and cold datasets

data_flagged_fvfm_final2 <- df_for_flagging_fvfm_final |>  # uses the Hot/Cold filtered dataframe
...
outlier_review_fvfm_final2 <- data_flagged_fvfm_final2 |> 
  filter(flag_iqr) |> # view only those with a flag for lo or hi 
  # choose which columns you would like to see in the summary df
  dplyr::select(all_of(group_vars_fin), leaf_nr, all_of(trait_var_fin), lo, hi)
...
x_levels <- levels(data_flagged_fvfm_final2$temp_exp)
...
if (all(c("month", "temp_exp", "age_class") %in% group_vars_fin)) {
  ...
  ggplot(data_flagged_fvfm_final2, # using flagged dataset
...
# create the base plots using one point estimate per plant (i.e. the 'leaf in the bath/freezer')
    ...
    geom_point(
      data = data_flagged_fvfm_final2 |>  filter(flag_iqr), # restate the df, and add filter
...
# create the base plots using one point estimate per plant (i.e. the 'leaf in the bath/freezer')
    ...
    labs(
      ...
      title = paste(trait_var_ini,
                    "Temperature Exposure by Month and Age Class (IQR flags)")
    ) +
...
df_for_flagging_wet_mass <- data |>     # remember to update name of df
  filter(proj_2025 == "Traits") |>      # filtering for hot and cold treatments
  filter(!is.na(wet_mass_leaf))         # filtering for any row that DO NOT have NA for wet_mass_leaf, the updated row containing mean wet mass based on correct nr of leaves at that stage


# defining the variable for outlier checking here 
trait_var_wm <- "wet_mass_leaf"   # here checking average wet mass per leaf
...
Now based on mean wet mass per individual leaf! **Outliers found.**
...
#need to get the data first, but the principle is the same as for the others 
```

### Leaf thickness individual rep IQR-flagging

```{r}
#| label: IQR-flagging leaf thickness rep
#| message: false

# choose the correct dataset for trait checking, here Traits for leaf thickness, 3 replicate measurements separate
# flagging the replicates that are outside the iqr range of the other replicates within the same month x age class x replicate plant_nr (x thickness_rep)

df_for_flagging_leaf_thickness_rep <- data |>     
  filter(proj_2025 == "Traits") |>      
...
# defining the variable for outlier checking here thickness rep
trait_var_lt_rep <- "leaf_thickness_mm"   # combined thickness rep measurements
                               

# defining relevant grouping variables
group_vars_lt_rep <- c("age_class", "month", "plant_nr", "thickness_rep")

# added "thichness_rep" here, so now only looking at iqr within the 5 replicates. If excluding it we'd be looking at 15 measurements (5 bulk leaves x 3 reps each) which makes more sense to me. Why compare eg. rep 1 for all 5 then rep 2 for all 5 etc. feel like it makes more sense to look at the outliers for all 15 values pooled. why not?

# calculating the Inter Quartile Range (IQR) for leaf thickness across replicates 

data_flagged_leaf_thickness_rep <- df_for_flagging_leaf_thickness_rep |>  
  group_by(across(all_of(group_vars_lt_rep))) |>  
  mutate(
...
# reviewing the 'flagged' rows
outlier_review_leaf_thickness_rep <- data_flagged_leaf_thickness_rep |> 
  filter(flag_iqr) |> 
...
if (all(c("month", "age_class") %in% group_vars_lt_rep)) {
  
  ggplot(data_flagged_leaf_thickness_rep,
         aes(x = month,
             y = .data[[trait_var_lt]])) +
    ...
    geom_point(
      data = data_flagged_leaf_thickness_rep |> filter(flag_iqr),
      position = position_jitter(width = 0.15),
      colour = "red",
      size = 2
    ) +
    ...
    labs(
      x = "Month",
      y = "Leaf thickness (mm)",
      title = "Leaf thickness reps grouped by Month and Age Class (with IQR flags)"
    ) +
...
#adding column for mean leaf thickness

data <- data |> 
 mutate(
  mean_leaf_thickness_mm = rowMeans(
    cbind(leaf_thickness_1_mm, leaf_thickness_2_mm, leaf_thickness_3_mm),
     na.rm = TRUE
  ))
...
df_for_flagging_leaf_thickness_mean <- data |>     
  ...
  filter(!is.na(mean_leaf_thickness_mm))   # remove missing means


# defining the variable for outlier checking
trait_var_lt_mean <- "mean_leaf_thickness_mm"
...
if (all(c("month", "age_class") %in% group_vars_lt_mean)) {
  ...
  ggplot(data_flagged_leaf_thickness_mean,
    ...
    labs(
      ...
      title = "Mean leaf thickness grouped by Month and Age Class (with IQR flags)"
    ) +
...
df_for_flagging_dry_mass <- data |>     
  filter(proj_2025 == "Traits") |>      
  filter(!is.na(dry_mass_leaf))             # filtering for any row that DO NOT have NA for dry_mass_g


# defining the variable for outlier checking here 
trait_var_dm <- "dry_mass_leaf"   # here checking dry mass
...
Now all of the above (except final_fvfm) also take plant_nr into account when grouping. More outliers now, but that makes sense as the range is probably smaller within each plant.
...
### Summary of errors/outliers

NEED TO DO THIS OVER AGAIN AFTER BULK_NR OF LEAVES AT EACH STEP IS TAKEN INTO ACCOUNT

(based on my understanding right now, and that my code works how it is supposed to)

-   There are 3 rows that needs to be dealt with after the quality control

    1\) DKU2239 - probably a wet mass typo. one decimal place wrong, would make sense compared to the dry mass. Think the right way to go here is to correct the value. Want to recode the value 0.001338 so that it is replaced with 0.01338, suggestion code:

    ```{r}
    #| label: correcting wrong wet mass 
    #| message: false

    # suggestion for how to code correcting decimal typo for DKU2239 0.001338 -> 0.01338
    ...
    This is the only value I have found that I am fairly certain is a typo, so also the only one to potentiall correct I think. Need to note somewhere, not sure best place.

    2\) nov pio 1, leaf 7 - NA for final fvfm

    3\) jan mat 4, leaf 14 - NA for final fvfm

    Would suggest to keep in clean dataset, but remove before analysis if reqired for it to work. know that both NAs come because of missing leaves at time of measurement. Should flag or write a comment w the reason, where is good? not sure yet
...
### SLA (specific leaf area) calculation

### LDMC (leaf dry matter content) calculation
