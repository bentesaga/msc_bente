...
data <- data |> 
  mutate(
    ...
    remark = case_when(
      ...
      remark == "leaf thickness 3 missing" ~ "only 2 leaf thickness values",
      TRUE ~ remark
    ),
...
# defining relevant grouping variables
group_vars_ini <- c("month", "age_class", "plant_nr")
...
# defining relevant grouping variables
group_vars_lt <- c("age_class", "month", "plant_nr")
...
-   The IQR-flagging highlighted outliers that possibly need to be dealt with. However, no impossible values as far as I can tell. Not sure if any needs recoding/removing. there is quite a few, though many are quite close to the iqr,

    Most outliers for fvfm final, quite a few that is far off (lower than range) from the iqr;
    ...
    jan pio 4, laf 14 - dead too early. same plant have outliers for other leaves too, but not as far off

    Some for fvfm initial too, these are much lower than the iqr.

    jan pio 4, leaf 16, 17, 19

    jan mat 3, leaf 20

Wet/dry mass and thickness, no values too far off the iqr, except DKU2239 as already mentioned
...
### Averaging leaf thickness values for each bulk leaf

not to be used just yet

```{r}
#| label: mean leaf thickness
#| message: false

 # find the mean of the leaf thickness values for each bulk leaf, make column
#data <- data |> 
#  mutate(
#    leaf_thickness_mm = rowMeans(
...
### Merging leaf scan data with main dataframe 
