...
Based on slack messages: With Fv/Fm being bounded (can be 0), better to use robust method (IQR, z-test) than QQ-plot or skewness. So will use IQR-flagging here.
...
-   There are 3 rows that needs to bea dealt with after the quality control

    1\) DKU2239 - probably a wet mass typo

    2\) nov pio 1, leaf 7

-   The IQR-flagging highlighted outliers that possibly need to be dealt with. Not sure how as there is qite a few.

-   Thec Q-Q plots and skewness tests (not sure how useful this actually was here) for wet, dry mass and for leaf thickness, shows that the distribution of the data within each month x age_class group is relatively close to a normal distribution. So I don't think there is any need for transformation of variables.
